...
function! <SID>DisplaySessions()
  ...
  let header = header . "\"\t'o' or Mouse-Double-Click: Delete all buffers and open the session under the cursor\n"
  let header = header . "\"\t'a' or Mouse-Double-Click: Add all buffers to the session under the cursor and open it\n"
  let header = header . "\"\t'e': Edit the quicksession config file `".g:quickSessionFile."`\n\n"
  silent! put! =header
  exe g:quickSessionLastPos
  nnoremap <buffer> <silent> o :call <SID>SelectSession(1)<cr>
  nnoremap <buffer> <silent> a :call <SID>SelectSession(0)<cr>
